...
24. que les contraintes avec et sans soient souples et influencent le score sans bloquer totalement la sortie.
25. que la contrainte souple sur l'équilibre de la taille des classes influence le score, afin que les classes restent proches en taille lorsque c'est possible.
26. que les contraintes fortes sur la capacité des classes, l'équilibre de la distribution et l'équilibre du comportement dominent le score, afin que les exigences critiques de l'école soient respectées en premier.
...
28. que l'application préserve les données importées et les résultats localement dans le navigateur, afin que je puisse rafraîchir ou revenir plus tard sans reconstruire tout immédiatement.
29. exporter le résultat final sous forme de CSV, afin que je puisse archiver ou partager facilement la proposition.
30. copier le résultat dans un format convivial pour les tableurs, afin que je puisse le coller directement dans mon outil de bureau habituel.
31. un bouton d'aide avec des exemples d'utilisation et des conventions, afin que je puisse me souvenir rapidement de la structure CSV attendue.
32. un bouton de contact pour les commentaires et les rapports de bugs, afin que je puisse proposer des améliorations sans quitter l'outil.
33. une interface d'une seule page, afin que je puisse compléter l'ensemble du workflow sans naviguer entre plusieurs écrans.
34. que l'application rende évidente la séquence de réglage des paramètres, d'importation, d'exécution et d'affichage des résultats, afin que le workflow reste simple.
35. que l'application renvoie une erreur claire lorsque le CSV d'entrée est mal formé, afin que je puisse corriger les problèmes avant de lancer l'optimisation.
36. savoir quelles sont les contraintes dures non satisfaites lorsque la solution est imparfaite, afin que je puisse juger si j'accepte ou ajuste l'entrée.
37. que le diagnostic de chaque proposition apparaisse juste au-dessus de son tableau de résultats, afin de lire immédiatement score et violations.
...
- L'application conserve les données et les résultats uniquement dans le navigateur.
...
- Les contraintes dures sont encodées avec une dominance de poids plus élevée que les contraintes souples dans la fonction objectif.
- Les contraintes avec, sans et groupés sont toujours considérées comme souples.
- Lorsqu'une proposition viole des contraintes dures, le diagnostic retourne pour chaque contrainte dure non respectée son type et les élèves concernés.
- Les types de contrainte dure diagnosticables sont appariement.
- Dans tous les cas, chaque proposition renvoyée inclut son score global et le nombre de contraintes souples non respectées.
...
- Le diagnostic affiché contient toujours score global et nombre de contraintes souples violées, et ajoute le diagnostic dur uniquement si applicable.
...
- La couche de diagnostic doit vérifier que chaque contrainte dure non respectée retourne bien un type parmi appariement et la liste des élèves concernés.
- La couche de résultats doit vérifier que chaque proposition contient toujours un score global et un nombre de contraintes souples non respectées.
...
- La couche d'interface doit vérifier que le diagnostic dur n'est affiché que lorsqu'il existe des contraintes dures violées.
...
- Le comportement de persistance du navigateur doit être testé à la limite de l'application en simulant un rechargement et en vérifiant que l'état local est préservé lorsque c'est supporté.
...
- Métriques d'équité avancées au-delà des contraintes dures et souples actuellement définies.
